...
#define FLUSH_LOG_FLAG 0x07
  if (buf[FLUSH_LOG_ARG_0] == FLUSH_LOG_FLAG &&
      buf[FLUSH_LOG_ARG_1] == FLUSH_LOG_FLAG) {
    return true;
  }
  return false;
}
...
#define MODULE_OFFSET 2
    ...
    ret = fprintf(fpWrite, "%02d-%02d %02d:%02d:%02d.%03d %d %d %s\n",
                  info->tm_mon + 1, info->tm_mday, info->tm_hour, info->tm_min,
                  info->tm_sec, head->nsec / NANOSEC_PER_MIRCOSEC, head->pid,
                  head->taskId, head->msg);
    if (ret < 0) {
