...
static int32 LogCommonFmt(char *outStr, int32 outStrlen,
                          const HiLogCommon *commonContentPtr);
static int32 LogValuesFmt(char *desStrPtr, int32 desLen,
                          const HiLogContent *logContentPtr);
static int32 LogDebugValuesFmt(char *desStrPtr, int32 desLen,
                               const HiLogContent *logContentPtr);
static int32 LogValuesFmtHash(char *desStrPtr, int32 desLen,
                              const HiLogContent *logContentPtr);
...
#ifndef DISABLE_HILOG_CACHE
  InitHiviewStaticCache(&g_logCache, LOG_CACHE, g_logCacheBuffer,
                        sizeof(g_logCacheBuffer));
#endif
...
void InitLogOutput(void) {
  ...
  if (InitHiviewFile(&g_logFile, type,
                     (HIVIEW_LOG_FILE_SIZE / sizeof(HiLogContent)) *
                         sizeof(HiLogContent)) == FALSE) {
    HILOG_ERROR(HILOG_MODULE_HIVIEW, "Open file[%d] failed.",
                HIVIEW_LOG_BIN_FILE);
  }
...
void OutputLog(uint8 module, const uint8 *data, uint32 len) {
  ...
  if (!isDisablePrintLimited && isLogLimited) {
    // The console output adopts the same restriction strategy as the file
    // output, and the log output to the file is restricted.
    return;
  }
...
static void OutputLogRealtime(const Request *req) {
  ...
  while (ReadFromCache(&g_logCache, (uint8 *)&(logContent.commonContent),
    ...
    if (len > 0 &&
        ReadFromCache(&g_logCache, (uint8 *)&(logContent.values), len) != len) {
      continue;
    }
...
uint32 GetLogFileSize(void) { return GetFileUsedSize(&g_logFile); }
...
static int32 LogCommonFmt(char *outStr, int32 outStrLen,
  ...
  ret = snprintf_s(outStr, outStrLen, outStrLen - 1,
                   "%04d-%02d-%02d %02d:%02d:%02d 0 %d %c %d/%s: ", year, month,
                   day, hour, min, sec, commonContentPtr->task,
...
static int32 LogDebugValuesFmt(char *desStrPtr, int32 desLen,
  ...
  case LOG_MULTI_PARA_0:
    ret = strncpy_s(desStrPtr, desLen, logContentPtr->commonContent.fmt,
                    desLen - 1);
    break;
  case LOG_MULTI_PARA_1:
    ret =
        snprintf_s(desStrPtr, desLen, desLen - 1,
                   logContentPtr->commonContent.fmt, logContentPtr->values[0]);
    break;
  case LOG_MULTI_PARA_2:
    ret = snprintf_s(desStrPtr, desLen, desLen - 1,
                     logContentPtr->commonContent.fmt, logContentPtr->values[0],
                     logContentPtr->values[1]);
    break;
...
static int32 LogValuesFmtHash(char *desStrPtr, int32 desLen,
  ...
  int32 len =
      snprintf_s(&desStrPtr[outLen], desLen - outLen, desLen - outLen - 1,
...
void HiviewRegisterHilogProc(HilogProc func) { g_hilogOutputProc = func; }
...
void HiLogOutputFileLockImp(void) { HIVIEW_MutexLock(g_outputLogInfo.mutex); }
