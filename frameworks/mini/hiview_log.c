...
#define LOG_IS_OUTPUT(mod)                                                     \
  (g_hiviewConfig.logOutputModule & (((uint64_t)1) << (mod)))
...
boolean HiLogRegisterModule(uint16 id, const char *name) {
  if ((id >= HILOG_MODULE_MAX) || name == NULL ||
      g_logModuleInfo[id].name != NULL) {
    return FALSE;
  }
...
void HiLogPrintf(uint8 module, uint8 level, const char *nums, const char *fmt,
  ...
  if (g_hiviewConfig.logSwitch == HIVIEW_FEATURE_OFF ||
      !CheckParameters(module, level) || !LOG_IS_OUTPUT(module)) {
    return;
  }
  ...
  OutputLog(module, (uint8 *)&logContent,
            sizeof(HiLogCommon) + sizeof(uint32) * argsNum);
}
...
void HiLogFlush(boolean syncFlag) { FlushLog(syncFlag); }

uint32 HiLogGetConfigOption(void) { return HiviewGetConfigOption(); }

void HiLogRegisterProc(HilogProc func) { HiviewRegisterHilogProc(func); }

void HiLogUnRegisterProc(HilogProc func) { HiviewUnRegisterHilogProc(func); }
...
void HiLogOutputFileLock(void) { HiLogOutputFileLockImp(); }

void HiLogOutputFileUnLock(void) { HiLogOutputFileUnLockImp(); }
